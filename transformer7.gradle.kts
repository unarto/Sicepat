...
rootProject {
    tasks.register("updateProxyCard") {
        doLast {
            val file = file("app/src/main/java/com/example/ProxiesScreen.kt")
            var code = file.readText()
            
            // Replace the Content of ProxyCard
            val proxyCardContentRegex = Regex("""Card\([\s\S]*?\)\s*\{[\s\S]*?Row\([\s\S]*?\}\n        \}\n    \}""")
            
            val newProxyCardContent = """
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .clickable { onClick() }
        ...
        if (isGrid) {
            ...
            code = code.replace(proxyCardContentRegex, newProxyCardContent)
            
            file.writeText(code)
            println("Replaced proxy card content!")
        }
    }
}
